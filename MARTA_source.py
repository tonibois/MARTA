...
class MyWindow:
    def __init__(self, win):
        ...
        self.lbl1=Label(win, text='Number of images (1 or 3)')
        self.t1=Entry(window,textvariable=StringVar(window, value='1'))
        self.lbl1.grid(column=0,row=4)
        self.t1.grid(column=1,row=4)
        CreateToolTip(self.t1, text = 'One merged input file (1) or three input files (3)')

        self.lbl2=Label(win, text='File label pattern (no ext)')
        self.t2=Entry(window,textvariable=StringVar(window, value='e'))
        self.lbl2.grid(column=0,row=5)
        self.t2.grid(column=1,row=5)
        CreateToolTip(self.t2, text = 'Pattern filename with no extension.')

        self.lb_33=Label(win, text='Input extension')
        self.t33=Entry(window,textvariable=StringVar(window, value='.tif'))
        self.lb_33.grid(column=2,row=5)
        self.t33.grid(column=3,row=5)

        self.lb_34=Label(win, text='Output extension')
        self.t34=Entry(window,textvariable=StringVar(window, value='.tif'))
        self.lb_34.grid(column=4,row=5)
        self.t34.grid(column=5,row=5)

        self.lbl4=Label(win, text='scaling (units/pixel)')
        self.t4=Entry(window,textvariable=StringVar(window, value='0.114'))
        ...
        btn2 = Button(window, text="Generate Automatic mask", command=self.run)
        ...
        btn2.grid(column=3, row=36)
        btn3.grid(column=4,row=36)
        btn10.grid(column=5, row=36)
        ...
        lbl5=Label(win, text='Equalized binary threshold')
        self.t5=Entry(window,textvariable=StringVar(window, value='70'))
        lbl5.grid(column=0,row=7)
        self.t5.grid(column=1,row=7)
        ...
        self.t5_2=Entry(window,textvariable=StringVar(window, value='254'))
        lbl5_2.grid(column=2,row=7)
        self.t5_2.grid(column=3,row=7)
        CreateToolTip(self.t5_2, text = '8-bit binary value for thresholding in c4 channel')

        lbl6=Label(win, text='Binary threshold for c1(CM)')
        self.t6=Entry(window,textvariable=StringVar(window, value='8'))
        lbl6.grid(column=0,row=8)
        self.t6.grid(column=1,row=8)
        CreateToolTip(self.t6, text = '8-bit binary value for thresholding in c1 channel (Myocite fill, i.e. SERCA, F-actin,...)')

        lbl7=Label(win, text='Binary threshold for c2(CX)')
        self.t7=Entry(window,textvariable=StringVar(window, value='15'))
        lbl7.grid(column=0,row=9)
        self.t7.grid(column=1,row=9)
        CreateToolTip(self.t7, text = '8-bit binary value for thresholding in c2 channel (CX43 for longitudinal CM delimitation)')

        lbl8=Label(win, text='Binary threshold for c3(INT)')
        self.t8=Entry(window,textvariable=StringVar(window, value='2'))
        lbl8.grid(column=0,row=10)
        self.t8.grid(column=1,row=10)
        CreateToolTip(self.t8, text = '8-bit binary value for thresholding in c3 channel (Intersticium fill, i.e. WGA)')

        lbl9=Label(win, text='Noise removal matrix rank in c1 (CM)')
        self.t9=Entry(window,textvariable=StringVar(window, value='1'))
        lbl9.grid(column=2,row=8)
        self.t9.grid(column=3,row=8)     

        lbl_10=Label(win, text='Noise removal matrix rank in c2 (CX)')
        self.t10=Entry(window,textvariable=StringVar(window, value='1'))
        lbl_10.grid(column=2,row=9)
        self.t10.grid(column=3,row=9)

        lb_11=Label(win, text='Noise removal matrix rank in c3 (INT)')
        self.t11=Entry(window,textvariable=StringVar(window, value='1'))
        lb_11.grid(column=2,row=10)
        self.t11.grid(column=3,row=10)

        lb_12=Label(win, text='Dilation matrix rank in c1 (CM)')
        self.t12=Entry(window,textvariable=StringVar(window, value='2'))
        lb_12.grid(column=4,row=8)
        self.t12.grid(column=5,row=8)

        lb_13=Label(win, text='Dilation matrix rank in c2 (CX)')
        self.t13=Entry(window,textvariable=StringVar(window, value='5'))
        lb_13.grid(column=4,row=9)
        self.t13.grid(column=5,row=9)

        lb_14=Label(win, text='Dilation matrix rank in c3 (INT)')
        self.t14=Entry(window,textvariable=StringVar(window, value='2'))
        lb_14.grid(column=4,row=10)
        self.t14.grid(column=5,row=10)

        lb_15=Label(win, text='Dilation iterations in c1 (CM)')
        self.t15=Entry(window,textvariable=StringVar(window, value='1'))
        lb_15.grid(column=6,row=8)
        self.t15.grid(column=7,row=8)

        lb_16=Label(win, text='Dilation iterations in c2 (CX)')
        self.t16=Entry(window,textvariable=StringVar(window, value='20'))
        lb_16.grid(column=6,row=9)
        self.t16.grid(column=7,row=9)

        lb_17=Label(win, text='Dilation iterations in c3 (INT)')
        self.t17=Entry(window,textvariable=StringVar(window, value='1'))
        lb_17.grid(column=6,row=10)
        self.t17.grid(column=7,row=10)

        lb_21=Label(win, text='First Filter minimum area (um**2)')
        self.t21=Entry(window,textvariable=StringVar(window, value='100'))
        lb_21.grid(column=0,row=11)
        self.t21.grid(column=1,row=11)

        lb_22=Label(win, text='First Filter minimum perimeter (um)')
        self.t22=Entry(window,textvariable=StringVar(window, value='40'))
        lb_22.grid(column=0,row=12)
        self.t22.grid(column=1,row=12)

        lb_23=Label(win, text='Second Filter L min (um)')
        self.t23=Entry(window,textvariable=StringVar(window, value='20'))
        lb_23.grid(column=0,row=13)
        self.t23.grid(column=1,row=13)

        lb_24=Label(win, text='Second Filter L max (um)')
        self.t24=Entry(window,textvariable=StringVar(window, value='200'))
        lb_24.grid(column=2,row=13)
        self.t24.grid(column=3,row=13)

        lb_25=Label(win, text='Second Filter W min (um)')
        self.t25=Entry(window,textvariable=StringVar(window, value='5'))
        lb_25.grid(column=0,row=14)
        self.t25.grid(column=1,row=14)

        lb_26=Label(win, text='Second Filter W max (um)')
        self.t26=Entry(window,textvariable=StringVar(window, value='50'))
        lb_26.grid(column=2,row=14)
        self.t26.grid(column=3,row=14)

        lb_27=Label(win, text='Second Filter R min')
        self.t27=Entry(window,textvariable=StringVar(window, value='1'))
        lb_27.grid(column=0,row=15)
        self.t27.grid(column=1,row=15)

        lb_270=Label(win, text='Padding scale factor')
        self.t270=Entry(window,textvariable=StringVar(window, value='1'))
        lb_270.grid(column=2,row=11)
        self.t270.grid(column=3,row=11)
        ...
        self.t28=Entry(window,textvariable=StringVar(window, value='0.5'))
        lb_28.grid(column=0,row=16)
        self.t28.grid(column=1,row=16)
        CreateToolTip(self.t28, text = 'Set < 1 to obtain brighter merged output or > 1 to obtain darker merged image output')
         
        lb_29=Label(win, text='Big scale bar lenght (um)')
        self.t29=Entry(window,textvariable=StringVar(window, value='100'))
        lb_29.grid(column=0,row=17)
        self.t29.grid(column=1,row=17)
        CreateToolTip(self.t29, text = 'Set the lenght of scale bar for full image')

        lb_30=Label(win, text='Small scale bar lenght (um)')
        self.t30=Entry(window,textvariable=StringVar(window, value='20'))
        lb_30.grid(column=2,row=17)
        self.t30.grid(column=3,row=17)
        CreateToolTip(self.t30, text = 'Set the lenght of scale bar for individual retrieval images of detections')

        lb_31=Label(win, text='Small window size (um)')
        self.t31=Entry(window,textvariable=StringVar(window, value='150'))
        lb_31.grid(column=0,row=18)
        self.t31.grid(column=1,row=18)
        CreateToolTip(self.t31, text = 'Set the size of the window for individual retrievals')
        
        lb_32=Label(win, text='Select CM ID plot')
        self.t32=Entry(window,textvariable=StringVar(window, value='all'))
        lb_32.grid(column=0,row=19)
        self.t32.grid(column=1,row=19)
        ...
        bo1 = Checkbutton(window, text='Plot ID numbers', var=self.bo1_state)
        ...
        bo7 = Checkbutton(window, text='Retrievals overlapped', var=self.bo7_state)
        ...
        bo9 = Checkbutton(window, text='Plot Individual CMs', var=self.bo9_state)
        ...
        lbl52=Label(win, text='*******   Parameters for EVALUATION  ***********')
        lbl52.grid(column=0,row=25)
        lbl521=Label(win, text='                                                ')
        lbl521.grid(column=0,row=26)
        
        lbl53=Label(win, text='Minimum Intersection value')
        self.t53=Entry(window,textvariable=StringVar(window, value='50'))
        lbl53.grid(column=0,row=27)
        self.t53.grid(column=1,row=27)

        lbl54=Label(win, text='Intersection mode (max/ref)')
        self.t54=Entry(window,textvariable=StringVar(window, value='ref'))
        lbl54.grid(column=0,row=28)
        self.t54.grid(column=1,row=28)

        lbl520=Label(win, text='                                                ')
        lbl520.grid(column=0,row=29)
        lbl52=Label(win, text='*******   Parameters for Manual Mask Generation  ***********')
        lbl52.grid(column=0,row=30)
        lbl521=Label(win, text='                                                ')
        lbl521.grid(column=0,row=31)
        
        lbl530=Label(win, text='Number of vertices of polygon')
        self.t530=Entry(window,textvariable=StringVar(window, value='8'))
        lbl530.grid(column=0,row=32)
        self.t530.grid(column=1,row=32)
        ...
        self.t531=Entry(window,textvariable=StringVar(window, value='1'))
        lbl531.grid(column=0,row=33)
        self.t531.grid(column=1,row=33)
        ...
        self.t532=Entry(window,textvariable=StringVar(window, value='1'))
        lbl532.grid(column=0,row=34)
        self.t532.grid(column=1,row=34)
    ...
    def run(self):
        ...
        f2 = open("out_quantif_"+dirstr+"/"+idim+"_"+distr+"_data.txt", 'w')

        if(procmode=='supervised'):
            f2.write('ID,value,L(um),W(um),ang(degree),CxLat2,AreaBox(um2),Area(um2),perimeter(um),xc(pix),yc(pix)\n')    
        else:
            f2.write('ID,L(um),W(um),ang(degree),CxLat2,AreaBox(um2),Area(um2),perimeter(um),xc(pix),yc(pix)\n')
        ...
        # In supervised mode revise
                    if(procmode=='supervised'):
                        ...
                        f2.write(str(cardiomdet)+','+str(value)+','+str(round(Lbox,2))+','+str(round(Wbox,2))+','+str(round(ang,2))+','+str(round((p1+p2)*100/(p0+p3+p1+p2+0.001),2))+','+str(round(Abox,2))+','+str(round(area,2))+','+str(round(perimeter,2))+','+str(round((x2+x1)*0.5,2))+','+str(round((y2+y1)*0.5,2))+'\n')
                        if(plotsepCMs=='y'):
                            if (miniscbar=='y'):
                                cv2.drawContours(origpic,[mscbar],-1,(255,255,255),-1)
                        ...
                        if(select=="all") & (value=='y'):
                            cv2.drawContours(img_contours,[rec1],0,(0,255,255),contwidth)
                            cv2.drawContours(img_contours,[rec2],0,(0,255,255),contwidth)
                            cv2.drawContours(img_contours,[rec3],0,(0,255,255),contwidth)
                            cv2.drawContours(img_contours,[rec4],0,(0,255,255),contwidth)
        ...
        # Plot ID numbers at center of rectangle if specified in parameters
                        ...
                        f2.write(str(cardiomdet)+','+str(round(Lbox,2))+','+str(round(Wbox,2))+','+str(round(ang,2))+','+str(round((p1+p2)*100/(p0+p3+p1+p2+0.001),2))+','+str(round(Abox,2))+','+str(round(area,2))+','+str(round(perimeter,2))+','+str(round((x2+x1)*0.5,2))+','+str(round((y2+y1)*0.5,2))+'\n')
                        angv.append(ang*sgang)
                        Lboxv.append(Lbox)
                        Wboxv.append(Wbox)
                        Aboxv.append(Abox)         
                        ...
                        if(plotsepCMs=='y'):
                            ...
                            cv2.drawContours(origpic,[rec1],0,(0,255,255),contwidth)
                            cv2.drawContours(origpic,[rec2],0,(0,255,255),contwidth)
                            cv2.drawContours(origpic,[rec3],0,(0,255,255),contwidth)
                            cv2.drawContours(origpic,[rec4],0,(0,255,255),contwidth)
                            
                            
                            cv2.drawContours(connexin,[rec1],0,(0,255,255),contwidth)
                            cv2.drawContours(connexin,[rec2],0,(0,255,255),contwidth)
                            cv2.drawContours(connexin,[rec3],0,(0,255,255),contwidth)
                            cv2.drawContours(connexin,[rec4],0,(0,255,255),contwidth)
        ...
        if(evaluate=='y'):
            ...
            f5.write('ID,L(um),W(um),ang(degree),CxLat2,AreaBox(um2),Max Intersection\n')
            print("************************************************************************************************************")
            print("This ouput has been generated on " + day +"th of " + month + " of year " + year + ", at local time:  " + time)
            print("************************************************************************************************************")
            print("******************   MARTA (Myocite Automatic Retrieval and Tissue Analyzer)   *****************************")
...
window.title('Myocite Automatic Retrieval and Tissue Analysis (MARTA)')
window.geometry("1600x800+10+10")
window.mainloop()
